<script setup>
import { auth, storage, songsCollection } from '@/utils/firebase'
...
// 设置当前浏览器默认不支持拖拽
const is_support_drag = ref(true)
const is_dragOver = ref(false)
const uploadList = ref([])

onMounted(() => {
  // 检测是否支持拖拽功能
  is_support_drag.value = test_support_drag()
})

const test_support_drag = () => {
  ...
  files.forEach((file) => {
    ...
    task.on(
      ...
      (snapshot) => {
        ...
        uploadList.value[currentIndex].current_progress = progress
      },
      (error) => {
        uploadList.value[currentIndex].variant = 'bg-red-400'
        uploadList.value[currentIndex].text_color = 'text-red-400'
        uploadList.value[currentIndex].icon = 'fas fa-times'
        console.log(error)
      },
      async () => {
        ...
        uploadList.value[currentIndex].variant = 'bg-green-400'
        uploadList.value[currentIndex].text_color = 'text-green-400'
        uploadList.value[currentIndex].icon = 'fas fa-check'
      }
    )
  })
}
